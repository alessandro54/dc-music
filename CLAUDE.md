...
- **Streaming pins `player_client=web_music`, overridable via `YTDLP_PLAYER_CLIENTS`.** Measured 2026-08-27 (cookied, direct): web_music + `use_ad_playback_context=true` pulls 1MB in **3.4–3.6s with zero mandated sleeps**, against 7.3–8.7s for the previous `web_embedded,default` pin — the ~4s gap is a **pre-roll ad**: yt-dlp stamps formats with `available_at` from the ad's skip-offset and *sleeps the ad off* before downloading ("Sleeping 4.00 seconds as required by the site"), and `adPlaybackContext:{pyv:true}` suppresses the placement. The flag rides `clientArgs` on every streaming call and is inert for clients without `SUPPORTS_AD_PLAYBACK_CONTEXT` (only web_music and mweb have it). **web_music does not carry everything** — gameplay and some old uploads answer "Video unavailable … not available on YouTube Music", failing fast (~1.6s) into the client escalation, where `web_embedded` (the old primary, still the proven general-audio server on this IP) picks them up at its usual ~7s; that recovery is logged, not captured — it is routine, not a dead pin. As of 2026-08-18, of the older set: `tv_downgraded` returns UNPLAYABLE, `web` returns URL-less formats (SABR), `mweb` extracts then 403s at download, `tv_simply`/`android_vr`/`ios` return storyboards or no formats (2026-08-27) — so **verify any client pin by pulling real bytes (≥1.5MB), not with `-g`**. The config var exists because this has broken twice from YouTube-side changes and a code push means image build + deploy — `dokku config:set music-bot YTDLP_PLAYER_CLIENTS=…` + restart is the 30-second version.
- **`createStream` escalates clients as a third attempt** (`web_music` → `web_embedded,default,mweb,ios`), after cookie-free and cookied both produce no audio. The pin is the piece most likely to rot, and when it does *every* play goes silent at once. It costs nothing in the common case: it only runs on a track already being dropped, so the trade is latency on a doomed play against the bot going mute until a new pin ships. A recovery captures to Sentry anyway — the primary pin being dead is news even when the fallback saves the play.
