...
- **Streaming pins `player_client=web_embedded,default`, overridable via `YTDLP_PLAYER_CLIENTS`.** As of 2026-08-18 the default set serves no playable audio on this IP: `tv_downgraded` returns UNPLAYABLE / "The page needs to be reloaded." and `web` returns URL-less formats (SABR). Only `web_embedded` yields bytes — verified on **14/14 real ids from prod logs**, cookied and cookie-free. `mweb` returns a URL that then 403s at download, so **verify any client pin by pulling real bytes, not with `-g`**; `default` stays last so a YouTube revert fixes itself. The config var exists because this has broken twice from YouTube-side changes and a code push means image build + deploy — `dokku config:set music-bot YTDLP_PLAYER_CLIENTS=…` + restart is the 30-second version.
- **`createStream` escalates clients as a third attempt** (`web_embedded,default` → `mweb,tv_simply,ios,android_vr`), after cookie-free and cookied both produce no audio. The pin is the piece most likely to rot, and when it does *every* play goes silent at once. It costs nothing in the common case: it only runs on a track already being dropped, so the trade is latency on a doomed play against the bot going mute until a new pin ships. A recovery captures to Sentry anyway — the primary pin being dead is news even when the fallback saves the play.
